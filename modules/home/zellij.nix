...
let cfg = config.cookie.zellij;

in with lib; {
  options.cookie.zellij = { enable = mkEnableOption "Enables zellij"; };

  config = mkIf cfg.enable {
    programs.zellij = { enable = true; };
    programs.bash.initExtra = ''
      function zellij_taboo {
        local dir="$XDG_RUNTIME_DIR/zellij-taboo"
        mkdir -p "$dir"
        touch "$dir/$SSH_CLIENT"
      ...
      # if [[ -z "$ZELLIJ" ]] && [ -n "$SSH_CLIENT" ]; then
      #   zellij_taboo
      # fi
    '';
    xdg.configFile."zellij/config.kdl".text = mkForce ''
