  ...
  paper = (pkgs.minecraft-server.overrideAttrs (oldAttrs: {
    src = pkgs.fetchurl {
      url = let
        build = 401;
        mc = "1.17.1";
      in "https://api.papermc.io/v2/projects/paper/versions/${mc}/builds/${
        toString build
      }/downloads/paper-${mc}-${toString build}.jar";
      sha256 = "sha256-Qpk/IAg5ExNJ5JbnTZRIZ3+7lRfG/oyafKz755ilf9o=";
    };
  })).override { jre_headless = pkgs.javaPackages.compiler.openjdk17; };
...
in with lib; {
  ...
  config = mkIf cfg.enable {
    ...
    cookie.restic = let
      mcExec = c:
        "${pkgs.systemd}/bin/systemctl is-active minecraft-server && ${console}/bin/mc ${
          escapeShellArg c
        }";
    in {
